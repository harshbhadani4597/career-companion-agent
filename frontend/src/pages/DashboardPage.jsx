...
export function DashboardPage({ profile, jobMatches, skillGaps, applications, setActiveTab, darkMode }) {
  ...
  return (
    <div className="space-y-8">
      {/* Welcome Banner */}
      <div className={`rounded-3xl p-8 shadow-xl transition-all ${
        darkMode
          ? "bg-gradient-to-r from-blue-900 via-indigo-950 to-slate-900 border border-blue-800/40 text-white"
        ...
        <div className="max-w-3xl">
          ...
          <div className="mt-6 flex flex-wrap gap-4">
            <button
              ...
              className="bg-white text-blue-900 hover:bg-blue-50 px-5 py-2.5 rounded-xl font-bold text-sm shadow-md transition-all"
            >
              📄 Upload / Update Resume
            </button>
            <button
              onClick={() => setActiveTab("matching")}
              className="bg-blue-600/80 hover:bg-blue-600 text-white px-5 py-2.5 rounded-xl font-bold text-sm shadow-md transition-all border border-blue-400/30"
            >
      ...
      {/* Metric Cards - Sleek Dark / Light rounded-3xl cards */}
      ...
      {/* Top Match Spotlight */}
      {topMatch && (
        <div className={`rounded-3xl p-6 sm:p-8 shadow-sm border relative overflow-hidden transition-all ${
          darkMode ? "bg-slate-900 border-purple-900/40 text-white" : "bg-white border-purple-100 text-slate-900"
        }`}>
          <div className="absolute top-0 right-0 bg-purple-600 text-white text-xs font-bold px-4 py-1.5 rounded-bl-2xl shadow">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                onClick={() => setActiveTab("matching")}
                className="bg-purple-600 hover:bg-purple-700 text-white px-5 py-3 rounded-2xl font-bold text-sm shadow-md transition-all"
              >
                View Details & Apply →
              </button>
            </div>
